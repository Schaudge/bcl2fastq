...
int main(int argc, const char *argv[])
{
    ...
    {
        BOOST_THROW_EXCEPTION(bcl2fastq::common::InputDataError(errnum, (boost::format("Unable to read header of cycle BCI file '%s': bytes_read=%d bytes_expected=%d") % bclFilePath.string() % blockSize % sizeof(maxBlockBytes)).str()));
    }
    fileBuf.close();

    // 2. output the bcl buffer into bgzf compressor
    std::vector<char> bclBgzfOutBuffer;
